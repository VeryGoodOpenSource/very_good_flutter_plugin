...
String expectedPlatformName() {
  {{#web}}if (isWeb) return 'Web';{{/web}}
  {{#android}}if (Platform.isAndroid) return 'Android';{{/android}}
  {{#ios}}if (Platform.isIOS) return 'iOS';{{/ios}}
  {{#linux}}if (Platform.isLinux) return 'Linux';{{/linux}}
  {{#macos}}if (Platform.isMacOS) return 'MacOS';{{/macos}}
  {{#windows}}if (Platform.isWindows) return 'Windows';{{/windows}}
